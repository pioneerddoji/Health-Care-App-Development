...
| app resume/lifecycle | URL listener 외에 `AppState` 구독은 현재 source에 없다. | not-proven, needs-device | background→foreground에서 stale UI/permission/notification state가 기대대로 refresh되는지 자동 증거가 없다. 필요 시 별도 구현 카드에서 lifecycle 계약과 device matrix를 추가한다. | 문서/preflight revert. |
